    ...
    class DistanceMatrix
          ...
          if part_origins.any?
            ...
            destinations_each_count.times do |limit_part_destinations|
              ...
              if part_destinations.any?
                ...
                response = GoogleMaps::Services::Connector.get_response 'distancematrix', request_params

                @status = response['status']
                raise GoogleMaps::Services::Exception, self.status if self.status != 'OK'
                ...
                response['origin_addresses'].each_with_index do |first_address, row_item|
                  response['destination_addresses'].each_with_index do |second_address, elem_item|
                    result << DistanceMatrix::Result.new({'origins' => first_address, 'destinations' => second_address}.merge(response['rows'][row_item]['elements'][elem_item]) )
                  end
                end
      ...
      def self.new_request(*args)
        if args.size == 1
          ...
          raise ArgumentError, 'GoogleMaps::Services::DistanceMatrix.new_request origins: args[0], destinations: args[1], key: args[2]'
        end
      end
      ...
      def self.new_request_hash(options = {})
        ...
        raise GoogleMaps::Services::Exception, distance_matrix.status if distance_matrix.status != 'OK'
